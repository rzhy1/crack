...
echo %ESC%[92m ===== 第一阶段：纯字典破解 ===== %ESC%[0m

for %%D in ("%DICTS_DIR%\*.lst" "%DICTS_DIR%\*.txt") do (
    echo.
    echo %ESC%[96m 🎯 当前字典: %%~nxD %ESC%[0m
...
REM ---------- 第二阶段：规则字典 ----------
echo.
echo %ESC%[93m ===== 第二阶段：字典 + Rule ===== %ESC%[0m

for %%D in ("%DICTS_DIR%\*.lst" "%DICTS_DIR%\*.txt") do (
    echo.
    echo %ESC%[96m 🎯 当前字典: %%~nxD %ESC%[0m

    hashcat -m !M! -a 0 -w 3 "%HASH_FILE%" "D:\Tools\Rar-password\hashcat\dicts\rockyou.txt" -r "%HASHCAT_DIR%\rules\office_fast.rule"
    hashcat -m !M! "%HASH_FILE%" --show 2>nul | findstr /R "^\$" >nul
    if !errorlevel! equ 0 (
        set "CRACKED=1"
        set "MODE_FOUND=!M!"
