...
REM 路径配置
REM =========================
set "JOHN_DIR=D:\Tools\Rar-password\john\run"
...
if /i "!FORMAT!"=="oldoffice" (
    john --format=!FORMAT! --incremental=digits --min-len=1 --max-len=5 "%JOHN_HASH%"
) else (
    john --format=!FORMAT!-opencl --incremental=digits --min-len=1 --max-len=5 "%JOHN_HASH%" --devices=1
)
john --show "%JOHN_HASH%" | find "1 password hash cracked" >nul && goto show_pwd

REM ========= 阶段2 =========
echo.
echo %ESC%[94m[阶段2] 6位纯数字%ESC%[0m
if /i "!FORMAT!"=="oldoffice" (
    john --format=!FORMAT! --incremental=digits --min-len=6 --max-len=6 "%JOHN_HASH%"
) else (
    john --format=!FORMAT!-opencl --incremental=digits --min-len=6 --max-len=6 "%JOHN_HASH%" --devices=1
)
john --show "%JOHN_HASH%" | find "1 password hash cracked" >nul && goto show_pwd
...
if /i "!FORMAT!"=="oldoffice" (
    echo %ESC%[94m[阶段3] 7-10位纯数字（仅oldOffice启用）%ESC%[0m
    john --format=!FORMAT!-opencl --incremental=digits --min-len=7 --max-len=10 "%JOHN_HASH%" --devices=1
    
    REM 成功就直接结束
    john --show "%JOHN_HASH%" | find "1 password hash cracked" >nul && goto show_pwd
    
    echo %ESC%[94m[阶段4] 3-6位小写字母和数字组合（仅oldOffice启用）%ESC%[0m
    john --format=!FORMAT!-opencl --mask=?1?1?1?1?1 -1="?l?d" --min-len=3 --max-len=6 "%JOHN_HASH%" --devices=1
    
    REM 再次检查是否成功
    john --show "%JOHN_HASH%" | find "1 password hash cracked" >nul && goto show_pwd
) else (
    echo %ESC%[94m[阶段3、4] 跳过（仅 oldoffice 启用）%ESC%[0m
)
