...
john --format=!FORMAT! --incremental=digits --min-len=1 --max-len=5 "!HASH_FILE!"
call :CHECK_CRACK && goto show_pwd

:: ---------- 阶段2 ----------
echo.
echo %ESC%[94m[阶段2] 6位纯数字%ESC%[0m
john --format=!FORMAT! --incremental=digits --min-len=6 --max-len=6 "!HASH_FILE!"
call :CHECK_CRACK && goto show_pwd

:: ---------- 阶段3（长耗时确认）----------
echo.
echo %ESC%[93m[阶段3] 7-10位纯数字%ESC%[0m
john --format=!FORMAT!-opencl --incremental=digits --min-len=7 --max-len=10 "!HASH_FILE!" --devices=1
call :CHECK_CRACK && goto show_pwd
...
for /l %%L in (3,1,6) do (
    set "MASK="
    for /l %%i in (1,1,%%L) do set "MASK=!MASK!?1"
    echo   长度 %%L ...
    john --format=!FORMAT!-opencl --mask="!MASK!" -1="?l?d" "!HASH_FILE!" --devices=1
    call :CHECK_CRACK && goto show_pwd
)
